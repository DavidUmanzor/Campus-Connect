	...
	try {
		...
		if (conflictCheck.rows.length > 0) {
			...
			return res.status(409).json({
				message: `Conflict with event from "${conflictEvent.rso_name}" - ${conflictEvent.location_name}, at ${conflictEvent.event_time}.`,
			});
		}
...
router.get("/rso/:rsoId", async (req, res) => {
	...
	try {
		const events = await pool.query("SELECT * FROM events WHERE rso_id = $1", [
			rsoId,
		]);
		res.json(events.rows);
	} catch (err) {
